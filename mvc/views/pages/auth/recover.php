    ...
    <script>
      $(document).ready(function () {
        $('#recoverForm').on('submit', function (e) {
          ...
          $loadingMessage.show();
          $btn.prop('disabled', true).html('<i class="fa fa-reply me-1"></i> Đang xử lý...');

          $.ajax({
            url: '/Quanlythitracnghiem/auth/sendOptAuth',
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            success: function (response) {
              ...
              if (response.success) {
                $recoverError
                  ...
                  .text(response.message || 'Yêu cầu khôi phục đã được gửi! Vui lòng kiểm tra email của bạn.')
                  .fadeIn();

                setTimeout(() => {
                  window.location.href = '/Quanlythitracnghiem/auth/signin';
                }, 2000);
              } else {
                $recoverError
                  .removeClass('alert-success')
                  .addClass('alert alert-danger')
                  .text(response.message || 'Không thể gửi yêu cầu khôi phục. Vui lòng thử lại.')
                  .fadeIn();

                $btn.prop('disabled', false).html('<i class="fa fa-reply me-1"></i> Khôi phục mật khẩu');
              }
            },
            error: function () {
              $loadingMessage.hide();
              $recoverError
                .removeClass('alert-success')
                .addClass('alert alert-danger')
                .text('Đã xảy ra lỗi khi gửi yêu cầu khôi phục!')
                .fadeIn();
              $btn.prop('disabled', false).html('<i class="fa fa-reply me-1"></i> Khôi phục mật khẩu');
            }
          });
