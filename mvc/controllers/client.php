...
class Client extends Controller
{
    ...
    public function joinGroup()
    {
        if ($_SERVER["REQUEST_METHOD"] == "POST" && AuthCore::checkPermission("tghocphan", "join")) {
            $mamoi = mysqli_real_escape_string($this->nhommodel->con, $_POST['mamoi']);
            $manguoidung = mysqli_real_escape_string($this->nhommodel->con, $_SESSION['user_id']);
            ...
            if ($result_manhom != null) {
                ...
                if ($result) {
                    ...
                    echo json_encode(1); // Lỗi: Sinh viên đã trong nhóm
                }
            } else {
                echo json_encode(0); // Lỗi: Mã mời không hợp lệ
            }
        } else {
            echo json_encode(-1); // Lỗi: Không có quyền hoặc không phải POST
        }
    }
